...
export function renderDebiteuren() {
  ...
  if (!s) {
    el('debiteuren-list').innerHTML = '<div style="padding: 20px; color: red;">State niet beschikbaar</div>';
    return;
  }
  ...
  const html = sorted.map(([naam, data]) => {
    ...
    return '<div class="debiteuren-row"><div class="debiteuren-naam">' + naam + '</div><div class="debiteuren-stats"><span class="debiteuren-count">' + data.count + 'x</span><span class="debiteuren-bedrag pos">+ €' + fmt(data.totaal) + '</span></div></div>';
  }).join('');
