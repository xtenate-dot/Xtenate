...
import { GBNM, ddmm, esc, fmt, isInkomst, isUitgave, leegVlak, rekBadge, teltBij, typeBadge, vulMaandSelect, weergaveNaam } from './helpers.js?v=20260820d';
import { state } from './storage.js?v=20260820d';
import { maakSorteerbaar } from './tables.js?v=20260820d';
...
const RUBRIEK_VOLGORDE = ['omzet', 'inkoop', 'kosten', 'prive', 'overig'];
...
function renderOverzicht(boekingen) {
  ...
  el('gb-body').innerHTML = rijen.length
    ? rijen.map(r => `<tr class="gb-rij" data-gb="${esc(r.gb)}" tabindex="0" role="button" aria-label="Bekijk boekingen van ${esc(r.gb)}">
